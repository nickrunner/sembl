...
Thirteen runnable examples under `src/examples/`, one per feature, over fictional listings in `data/`. `src/support/provider.ts` picks Anthropic or OpenAI from the env and wraps it in `replayOrRecord` so runs are recorded under `recordings/` (gitignored). `pnpm --filter @sembl/examples demo [n]`.
